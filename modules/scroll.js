...
function efeitosSecaoLancamentos (scrollDocumento) {
    ...
    if (scrollDocumento >= ( titulo.offsetTop - 500 )) 
        classesDosElementos.adicionar('on-scroll' , titulo)

    if (scrollDocumento >= ( carrosssel.offsetTop - 400 )) 
        classesDosElementos.adicionar('on-scroll' , carrosssel)
}
...
function efeitosSecaoMarketing (scrollDocumento) {
    ...
    if (scrollDocumento >= ( bannerProdutos.offsetTop - 450 )) 
        classesDosElementos.adicionar('on-scroll', bannerProdutos)

    if (scrollDocumento >= ( bannerIlustrativo.offsetTop - 450 )) 
        classesDosElementos.adicionar('on-scroll', bannerIlustrativo)
}


function efeitosSecaoProdutosOferta (scrollDocumento) {
    const gridProdutos = document.querySelector('.produtos-oferta__grid')
    if (scrollDocumento >= ( gridProdutos.offsetTop - 450 )) 
        classesDosElementos.adicionar('on-scroll' , gridProdutos)
}


function efeitosSecaoCategorias (scrollDocumento) {
    const gridCategorias = document.querySelector('.categorias__flex')
    if (scrollDocumento >= ( gridCategorias.offsetTop - 450 )) 
        classesDosElementos.adicionar('on-scroll' , gridCategorias)
}
...
function efeitosSecaoMapa (scrollDocumento) {
    ...
    if (scrollDocumento >= ( mapa.offsetTop - 350 )) 
        classesDosElementos.adicionar('on-scroll' , infoMapa)
}